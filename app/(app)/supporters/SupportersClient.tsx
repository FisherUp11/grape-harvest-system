...
export default function SupportersClient({ profile, profiles, supporters, receipts, periods }: Props) {
  ...
  const createSupporter = async (e: React.FormEvent) => {
    e.preventDefault();
    const { error } = await supabase.from("supporters").insert({ org_id: profile.org_id, owner_id: profile.id, name, phone: phone || null, email: email || null, address: address || null });
    if (error) return setMessage(error.message);
    setName(""); setPhone(""); setEmail(""); setAddress(""); router.refresh();
  };

  const exportCsv = () => {
    downloadCsv(`支持者明细_${new Date().toISOString().slice(0,10)}.csv`, [["用户", "支持者", "电话", "邮箱", "地址", "支持类型", "周期", "已确认葡萄", "待确认葡萄", "未收到葡萄", "次数", "最近收到"], ...supporters.map((s) => [profileName(profileMap[s.owner_id]), s.name, s.phone || "", s.email || "", s.address || "", s.support_type === "recurring" ? "周期性" : "一次性", cycleLabel(s.cycle), String(stats[s.id]?.confirmed || 0), String(stats[s.id]?.pending || 0), String(stats[s.id]?.missed || 0), String(stats[s.id]?.count || 0), stats[s.id]?.last || ""])]);
  };
    ...
    <>
      ...
      <section className="grid grid-2">
        <form className="card stack" onSubmit={createSupporter}>
          <h2>新增支持者</h2>
          <div className="form-grid"><div className="field"><label>姓名</label><input className="input" value={name} onChange={(e) => setName(e.target.value)} required /></div><div className="field"><label>电话</label><input className="input" value={phone} onChange={(e) => setPhone(e.target.value)} /></div><div className="field"><label>邮箱</label><input className="input" type="email" value={email} onChange={(e) => setEmail(e.target.value)} /></div><div className="field"><label>地址</label><input className="input" value={address} onChange={(e) => setAddress(e.target.value)} /></div></div>
